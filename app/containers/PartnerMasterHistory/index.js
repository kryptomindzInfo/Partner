...
export default class PartnerOperationalHistory extends Component {
  constructor() {
    super();
    this.state = {
      bank_id: '',
  ...
  getHistory = () => {
    axios
      .post(`${API_URL}/partner/getHistory`, {
        token: token,
        from: 'master',
  ...
  getHistoryTotal = () => {
    axios
      .post(`${API_URL}/partner/getHistoryTotal`, {
        token: token,
        from: 'master',
      })
      .then(res => {
  ...
  render() {
    ...
    return (
      <Wrapper>
        <Helmet>
          <meta charSet="utf-8" />
          <title>Master History | Partner | E-WALLET</title>
        </Helmet>
        <PartnerHeader />
